...
class IncrementalOutputForm(KMPC):
    ...
    def compute_control(self, x, y_ref):
        ...
        x_curr_scaled = self.model.scaler_x.transform(x.reshape(1, -1))
        z_curr_scaled = self.model.lift(x_curr_scaled).flatten()
        y_ref_scaled = self.model.scaler_y.transform(y_ref_batch)
        
        # Set initial condition for solver
        x0_t = np.concatenate([z_curr_scaled, self._u_prev_scaled])
        ...
        terminal_yref = np.concatenate([np.zeros_like(y_ref_scaled[-1]), zeros_u])
        self._solver.set(self.mpc_params.N_horizon, "yref", terminal_yref)
