...
class Standard(KLQR):
    ...
    def _dare_solution(self):
        ...
        if self._logger:
            ...
            if min_eig > -1e-10:
                self._logger.info(f"[{self._node_name}] DARE P is stable (min eig: {min_eig:.2e})")
            else:
                self._logger.warning(f"[{self._node_name}] DARE P has negative eigenvalue: {min_eig:.2e}")
